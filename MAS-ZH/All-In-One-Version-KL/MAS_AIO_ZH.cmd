...
:: 针对 https://github.com/microsoft/terminal/issues/15212 的解决方法，当 %0 以引号开头时，%0 参数展开不会特殊处理。
:: 将 %0 改为不带引号的内容可以绕过此问题。
...
:: 如果在 x64 位 Windows 上由 x86 进程启动，则使用 x64 进程重新启动脚本
:: 如果在 ARM64 Windows 上由 x86/ARM32 进程启动，则使用 ARM64 进程重新启动脚本
...
echo:
echo:
echo 请查看此网页获取帮助 - %mas%fix_service
echo:
echo:
ping 127.0.0.1 -n 20
)
...
>nul findstr /v "$" "%_cmdf%" && (
echo:
echo 错误 - 脚本存在 LF 行尾问题或脚本末尾缺少空行。
echo:
echo:
echo 请查看此网页获取帮助 - %mas%troubleshoot
echo:
echo:
...
if %winbuild% EQU 1 (
%eline%
echo 无法检测 Windows 内部版本号。
echo:
setlocal EnableDelayedExpansion
set fixes=%fixes% %mas%troubleshoot
call :dk_color2 %Blue% "请查看此网页获取帮助 - " %_Yellow% " %mas%troubleshoot"
goto dk_done
)
...
%eline%
echo 脚本是从临时文件夹启动的。
echo 您很可能直接从存档文件中运行脚本。
echo:
echo 请解压缩存档文件，然后从解压后的文件夹中启动脚本。
goto dk_done
)
)
...
call :dk_color2 %Blue% "请查看此网页获取帮助 - " %_Yellow% " %mas%fix_powershell"
goto dk_done
)
...
echo MAS 需要 Windows Powershell，但它似乎被 Powershell core 取代了。正在中止...
set fixes=%fixes% %mas%in-place_repair_upgrade
call :dk_color2 %Blue% "请查看此网页获取帮助 - " %_Yellow% " %mas%in-place_repair_upgrade"
goto dk_done
)

REM 检查可能导致 Powershell 问题的恶意软件

for /r "%ProgramFiles%\" %%f in (secureboot.exe) do if exist "%%f" (
echo "%%f"
echo 发现恶意%blank%软件，PowerShell 无法正常工作。
set fixes=%fixes% %mas%remove_mal%w%ware
call :dk_color2 %Blue% "请查看此网页获取帮助 - " %_Yellow% " %mas%remove_mal%w%ware"
goto dk_done
)
...
if /i "!tstresult2!"=="FullLanguage" (
echo:
echo 您的防病毒软件可能正在阻止此脚本。
echo:
sc query sense | find /i "RUNNING" %nul% && (
echo 已安装防病毒软件 - Microsoft Defender for Endpoint
)
cmd /c "%psc% ""$av = Get-WmiObject -Namespace root\SecurityCenter2 -Class AntiVirusProduct; $n = @(); foreach ($i in $av) { $n += $i.displayName }; if ($n) { Write-Host ('已安装防病毒软件 - ' + ($n -join ', '))}"""
)
...
::  禁用 QuickEdit 并从 conhost.exe 启动，避免使用终端应用
...
::  重新启动以禁用当前会话中的 QuickEdit 并使用 conhost.exe 而不是终端应用
::  此代码禁用当前 cmd.exe 会话的 QuickEdit，而不更改永久注册表设置
::  包含它是为了防止点击脚本窗口暂停执行，导致用户误以为脚本因错误而停止
...
call :dk_color2 %_Green% "       提示:" %_White% " Windows 10 结束支持后，如需激活 ESU 扩展安全更新，请使用 TSforge 选项。"
)
echo:
echo:
echo:
...
if %_erl%==4 (set "_oem=在线 KMS" & set "para=/K-WindowsOffice" &goto:Extract$OEM$3)
...
:: 在顶部添加随机数据以创建唯一文件，有助于避免防病毒软件的检测
...
::  要激活，请使用 "/HWID" 参数运行脚本或将下面行中的 0 改为 1
set _act=0

::  如果当前版本不支持 HWID 激活，要禁用更改版本，请将值从 0 改为 1 或使用 "/HWID-NoEditionChange" 参数运行脚本
...
call :dk_color %Blue% "从主菜单使用 TSforge 激活选项。"
goto dk_done
)
...
call :dk_color %Blue% "返回主菜单，选择“故障排除”并运行 DISM 还原和 SFC 扫描选项。"
call :dk_color %Blue% "之后，重新启动系统并再次尝试激活。"
set fixes=%fixes% %mas%in-place_repair_upgrade
call :dk_color2 %Blue% "如果仍然显示相同的错误，请执行此操作 - " %_Yellow% " %mas%in-place_repair_upgrade"
)
goto dk_done
)
)
...
echo [%winos% ^| %winbuild%]
echo:
echo 评估版本无法在其评估期之外激活。
call :dk_color %Blue% "从主菜单使用 TSforge 激活选项以重置评估期。"
set fixes=%fixes% %mas%evaluation_editions
call :dk_color2 %Blue% "请查看此网页获取帮助 - " %_Yellow% " %mas%evaluation_editions"
goto dk_done
)
)
...
call :dk_color %Blue% "HWID 激活需要互联网。"
)
...
call :dk_color %Red% "检查备用版本的 HWID     [%altedition% 激活 ID 未找到]"
)
...
call :dk_color %Blue% "将使用 [%altedition%] 版本产品密钥来启用 HWID 激活。"
echo:
)
...
::  将 Windows 区域更改为美国，以避免激活问题，因为 Windows 商店许可证在许多国家/地区不可用
...
if !errorlevel! EQU 0 (
echo 将 Windows 区域更改为美国          [成功] [脚本会将其改回]
) else (
...
::  生成并应用 GenuineTicket.xml
::  在某些情况下 clipup -v -o 方法失败，在某些情况下服务重启方法也失败
::  为最大化成功率并获得更好的错误详情，脚本将安装票证两次（服务重启 + clipup -v -o）
...
set error=1
if exist "%tdir%\*.xml" del /f /q "%tdir%\*.xml" %nul%
call :dk_color %Gray% "安装 GenuineTicket.xml            [通过 ClipSVC 服务重启失败，请等待...]"
)
)
...
set error=1
set rebuildinfo=1
call :dk_color %Red% "安装 GenuineTicket.xml            [通过 clipup -v -o 失败]"
)
...
findstr /i "%%#" "%SysPath%\drivers\etc\hosts" %nul1% && set "hosfail= [%%# 在 Hosts 中被阻止]"
)
call :dk_color %Red% "检查许可服务器              [连接失败]!hosfail!"
set fixes=%fixes% %mas%licensing-servers-issue
call :dk_color2 %Blue% "请查看此网页获取帮助 - " %_Yellow% " %mas%licensing-servers-issue"
echo:
)
...
if defined wublock (
call :dk_color %Red% "检查注册表中的更新阻止程序     [已找到]"
call :dk_color %Blue% "HWID 激活需要正常的 Windows 更新，如果您使用了任何阻止更新的工具，请撤销它。"
echo:
)

reg query "HKLM\SOFTWARE\Policies\Microsoft\WindowsStore" /v DisableStoreApps %nul2% | find /i "0x1" %nul% && (
set storeblock=1
call :dk_color %Red% "检查注册表中的商店阻止程序      [已找到]"
call :dk_color %Blue% "如果您使用了任何阻止商店的工具，请撤销它。"
echo:
)
...
set error=1
call :dk_color %Red% "检查 Windows 更新注册表        [发现损坏]"
if !wcount! GTR 2 (
call :dk_color %Red% "Windows 似乎感染了恶意%w%软件。"
set fixes=%fixes% %mas%remove_mal%w%ware
call :dk_color2 %Blue% "请查看此网页获取帮助 - " %_Yellow% " %mas%remove_mal%w%ware"
echo:
) else (
...
call :dk_color %Red% "检查互联网问题                [已找到] %error_code%"
set fixes=%fixes% %mas%licensing-servers-issue
call :dk_color2 %Blue% "请查看此网页获取帮助 - " %_Yellow% " %mas%licensing-servers-issue"
echo:
)
...
call :dk_color %Blue% "在撰写本文时，此产品不支持 HWID 激活。"
...
if !errorlevel! EQU 0 (
echo 恢复 Windows 区域                [成功]
) else (
call :dk_color %Red% "恢复 Windows 区域                [失败] [%name% - %nation%]"
)
)
...
echo 检查 OS 信息                        [%winos% ^| %fullbuild% ^| %osarch%]
exit /b
...
::  当用户手动安装了过多 Office 许可证时，检查潜在的脚本崩溃问题（变量长度限制）
...
::  此密钥由系统在重设过程中留下，sppsvc 有时无法删除它，这会导致 SPP 计划任务出现问题
...
::  使用 Powershell/WMI 而不是 slmgr.vbs 安装许可证文件
...
call :dk_color %Blue% "使用重启选项重新启动您的机器，然后重试。"
call :dk_color %Blue% "如果仍然无法工作，返回主菜单，选择“故障排除”并运行“修复 WPA 注册表”选项。"
)
)

echo:
%psc% "$job = Start-Job { (Get-WmiObject -Query 'SELECT * FROM %sps%').Version }; if (-not (Wait-Job $job -Timeout 30)) {write-host '%_slser% 无法正常工作。请查看此网页获取帮助 - %mas%troubleshoot'}"
exit /b

::  获取产品名称（WMI/REG 方法并非在所有情况下都可靠，因此使用 winbrand.dll 方法）
...
::  如果找到 Windows 订阅许可证，则检查基础版本产品名称
...
::  许多用户通过 Google 搜索无意中下载了恶意软件。
::  此代码旨在通知用户其系统已受到恶意软件影响。
...
if %hcount%==4 set "results=[防病毒 URL 在 hosts 中被阻止]"

sc start %_slser% %nul%
echo "%errorlevel%" | findstr "577 225" %nul% && (
set "results=%results%[可能是文件感染器]"
) || (
if not exist %SysPath%\%_slexe% if not exist %SysPath%\alg.exe (set "results=%results%[可能是文件感染器]")
)

if not "%results%%pupfound%"=="" (
if defined pupfound call :dk_color %Gray% "检查 PUP 激活程序                 [已找到%pupfound%]"
if defined results call :dk_color %Red% "检查可能的恶意%w%软件感染..."
...
::  删除 R@1n-KMS（旧版本）的计划任务，该任务每分钟运行一次激活命令，导致 CPU 使用率过高。
...
sc query Null %nul% || (
call :dk_color %Red% "检查沙箱                     [已找到，脚本可能无法正常工作]"
if not defined showfix (
call :dk_color %Blue% "如果您使用的是第三方防病毒软件，请检查它是否阻止了脚本。"
echo:
)
set error=1
set showfix=1
...
call :dk_color %Red% "检查 WinPE                          [已找到]"
if not defined showfix (
call :dk_color %Blue% "检测到 WinPE 模式。请重新启动系统并以正常模式运行。"
echo:
)
set error=1
set showfix=1
...
if not defined showfix (
call :dk_color %Blue% "检测到安全模式。请重新启动系统并以正常模式运行。"
echo:
)
set error=1
set showfix=1
...
if not defined showfix (
call :dk_color %Blue% "如果您在审核模式下运行，您需要在正常模式下运行它。"
echo:
)
set error=1
set showfix=1
...
call :dk_color2 %Blue% "请查看此网页获取帮助 - " %_Yellow% " %mas%fix_service"
) else (
set fixes=%fixes% %mas%in-place_repair_upgrade
call :dk_color2 %Blue% "请查看此网页获取帮助 - " %_Yellow% " %mas%in-place_repair_upgrade"
)
...
echo 已应用注册表修复以启用禁用的服务。
call :dk_color %Blue% "使用重启选项重新启动您的机器以修复此错误。"
) || (
set fixes=%fixes% %mas%in-place_repair_upgrade
call :dk_color2 %Blue% "请查看此网页获取帮助 - " %_Yellow% " %mas%in-place_repair_upgrade"
)
...
::  检查服务是否能够运行
::  添加了解决方法以获取正确的状态和错误代码，因为 sc query 在某些情况下不输出正确结果
...
set showfix=1
call :dk_color %Blue% "使用重启选项重新启动您的机器，然后再次运行脚本。"
set fixes=%fixes% %mas%in-place_repair_upgrade
...
echo "%error_code%" | findstr /i "0x800410 0x800440 0x80131501" %nul1% && set wmifailed=1& ::  https://learn.microsoft.com/zh-cn/windows/win32/wmisdk/wmi-error-constants

if defined wmifailed (
call :dk_color %Red% "检查 WMI                            [无法正常工作]"

if not defined showfix (
call :dk_color %Blue% "返回主菜单，选择“故障排除”并运行“修复 WMI”选项。"
echo:
)
set error=1
set showfix=1
...
call :dk_color %Red% "检查 SPP 注册表项               [找到错误的 ModuleId] [很可能由游戏欺骗程序引起]"
if not defined showfix (
set fixes=%fixes% %mas%issues_due_to_gaming_spoofers
call :dk_color2 %Blue% "请查看此网页获取帮助 - " %_Yellow% " %mas%issues_due_to_gaming_spoofers"
echo:
)
set error=1
set showfix=1
...
if exist "%tokenstore%\" (
call :dk_color %Gray% "检查 SPP 令牌文件夹               [未找到，现已创建] [%tokenstore%\]"
) else (
...
::  此代码检查 SPP 是否有权访问令牌文件夹和所需的注册表项。通常由游戏欺骗程序引起。
...
REM  https://learn.microsoft.com/zh-cn/office/troubleshoot/activation/license-issue-when-start-office-application
...
call :dk_color %Blue% "返回主菜单，选择“故障排除”并运行“修复 WPA 注册表”选项。"
echo:
set error=1
set showfix=1
)
...
call :dk_color %Red% "检查 Rearm                          [系统已 Rearm]"
if not defined showfix (
call :dk_color %Blue% "使用重启选项重新启动您的机器以修复此错误。"
echo:
)
...
call :dk_color %Red% "检查 ClipSVC PersistedSystemState   [已找到]"
if not defined showfix (
call :dk_color %Blue% "使用重启选项重新启动您的机器以修复此错误。"
echo:
)
...
::  检查 SoftwareLicensingService

if %error_code% NEQ 0 (
call :dk_color %Red% "检查 SoftwareLicensingService       [无法正常工作] [%error_code%]"
if not defined showfix (
call :dk_color %Blue% "%_fixmsg%"
call :dk_color %Blue% "如果激活仍然失败，则运行“修复 WPA 注册表”选项。"
echo:
)
set error=1
set showfix=1
...
call :dk_color %Gray% "检查激活 ID                 [未安装密钥或未找到 Act ID]"
)
...
reg query "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion" /v EditionID %nul2% | find /i "Eval" %nul1% || (
call :dk_color %Red% "检查 Eval 包                  [发现许可证交换。在 Eval Windows 中安装了非 Eval 许可证]"
if not defined showfix (
call :dk_color %Blue% "许可证交换不是升级到完整版的正确方法。请通过以下链接了解正确方法。"
set fixes=%fixes% %mas%evaluation_editions
call :dk_color2 %Blue% "请查看此网页获取帮助 - " %_Yellow% " %mas%evaluation_editions"
echo:
)
...
::  检查 HKU\S-1-5-20\Software 注册表，在某些系统中它缺失，这会导致 Windows 激活问题
...
::  针对 1607 和 1709 之间版本中 ProfessionalEducation 显示为 Professional 的问题的解决方法
...
if %osedition%==0 (
call :dk_color %Red% "检查版本名称                   [在注册表中未找到]"
) else (
...
::  此“WLMS”服务包含在以前的评估版本中（这些版本是可激活的），用于在评估期过后每小时自动关闭系统并防止 SPPSVC 停止。

sc query wlms %nul%

if %errorlevel% NEQ 1060 (
echo 检查 Eval WLMS 服务              [已找到]
)
...
call :dk_color %Red% "检查 IFEO 中的 SPP 干扰       [%_sppint% - 系统可能会在稍后停用]"
if not defined showfix (
call :dk_color %Blue% "%_fixmsg%"
echo:
)
...
echo 检查 IFEO 中的 SPP                    [%_sppint%]
)
)

::==============================

::  检查并修复 SkipRearm 注册表值
...
::  检查 SvcRestartTask 状态，此任务有助于确保系统保持激活状态
...
call :dk_color %Gray% "检查 SvcRestartTask 状态          [!taskinfo!. 系统可能会在稍后停用。]"
if not defined showfix (
...
::  此代码检查 HKLM\SYSTEM\WPA 中的无效注册表项。即使在健康的系统上也可能会出现此问题
...
if defined fixes (
call :dk_color %White% "请遵循上述所有蓝色提示行。   "
...
::  第 3 列 = SKU ID
::  第 4 列 = 密钥部件号
::  第 5 列 = 1 = 激活不工作（在撰写本文时），0 = 激活工作
::  第 6 列 = 密钥类型
::  第 7 列 = WMI 版本 ID（仅供参考）
::  第 8 列 = 版本名称，以防相同的版本 ID 在不同的 OS 版本中与不同的密钥一起使用
::  分隔符  = _
...
::  如果当前版本不支持 HWID 激活，则使用下面的代码获取备用版本名称和密钥
...
::  要使用 Ohook 激活 Office，请使用 "/Ohook" 参数运行脚本或将下面行中的 0 改为 1
set _act=0

::  要卸载 Ohook 激活，请使用 /Ohook-Uninstall 参数运行脚本或将下面行中的 0 改为 1
set _rem=0
...
echo:
echo:
if defined checknames (call :dk_color %_Yellow% "                继续前请关闭 [!checknames!]...")
echo         ____________________________________________________________
echo:
...
echo         ____________________________________________________________
echo:
call :dk_color2 %_White% "             " %_Green% "使用键盘选择菜单选项 [1,2,3,0]"
choice /C:1230 /N
set _el=!errorlevel!
if !_el!==4  exit /b
...
call :dk_color %Red% "检查 ClickToRun 服务             [未找到，但发现 Office 16.0 文件]"
set o16c2r=
set error=1
)
...
call :dk_color %Red% "检查 ClickToRun 服务             [未找到，但发现 Office 15.0 文件]"
set o15c2r=
set error=1
)
...
if defined ohub (
echo:
echo 您只安装了 Office Dashboard 应用；您需要安装完整版 Office。
)
...
call :dk_color %Gray% "检查多个 Office 安装        [已找到，建议只安装一个版本]"
)
...
if defined sacstate (
if "%sacstate%"=="0x1" (
call :dk_color %Gray% "检查 Smart App Control 状态        [已启用]"
call :dk_color %Blue% "Smart App Control 可能会阻止您在 Ohook 激活后打开 Office。"
call :dk_color %Blue% "如果发生这种情况，您需要从 Windows Defender 设置中禁用它。"
) else if "%sacstate%"=="0x2" (
call :dk_color %Gray% "检查 Smart App Control 状态        [评估中]"
call :dk_color %Blue% "Smart App Control 可能会在评估期后自行启用，从而阻止您在将来打开 Office。"
...
::  带有订阅许可证密钥的旧版 Office 可能会显示登录横幅以修复许可证问题。
::  尽管脚本应用了 Resiliency 注册表项来修复此问题，但它不适用于旧版 Office。
::  下面的代码检查该条件并通知用户更新 Office。

if defined _sublic (
if not exist "%_oLPath%\Word2021VL_KMS_Client_AE*.xrm-ms" (
call :dk_color %Gray% "检查带有订阅许可证的旧版 Office    [已找到。请更新 Office，否则可能会显示与许可问题相关的横幅。]"
)
)
...
::  为批量产品添加注册表项，以防止显示“非正版”横幅
::  脚本已经使用 MAK 而不是 GVLK，因此它不会出现，但仍会添加注册表项，以防 Office 为批量产品安装默认的 GVLK 宽限期密钥
...
if defined ohub call :dk_color %Gray% "Word、Excel 等 Office 应用已激活，请直接使用它们。忽略 Office 仪表板应用中的“购买”按钮。"
...
call :dk_color %Blue% "如果仍未修复，请使用重启选项重新启动您的机器并重试。"
) else (
call :dk_color %Blue% "使用重启选项重新启动您的机器并重试。"
)
) else (
...
set error=1
set showfix=1
call :dk_color %Red% "检查过期的预览产品       [找到 Office %%# 预览版]"
...
::  一些 Office 零售版到批量版的转换工具可能会编辑 ProductReleaseIds 以添加 VL 产品。此代码将其恢复，因为它可能会影响功能。
...
::  如果 integrator.exe 不工作，则回退到手动安装许可证的方法
...
if exist "%_hookPath%\%%#" (set "ierror=删除以前的 Ohook 安装 [%%#]")
)
...
echo:
echo 符号链接系统的 sppc.dll            ["%_hookPath%\sppcs.dll"] [成功]
...
if exist "%%~A" (set "ierror=删除以前的 Ohook 安装 [%%#]")
)
)
)
...
if exist "%_hookPath%\sppcs.dll" (set "ierror=删除以前的 Ohook mklink sppcs.dll")
...
echo:
if defined _osppPath68 (echo 将 OSPPC.DLL 重命名为 sppcs.dll         ["%_osppPath68%\sppcs.dll"])
if defined _osppPath86 (echo 将 OSPPC.DLL 重命名为 sppcs.dll         ["%_osppPath86%\sppcs.dll"])
if defined _osppPath68 (echo 提取自定义 %_hook68% 到         ["%_osppPath68%\OSPPC.DLL"])
if defined _osppPath86 (echo 提取自定义 %_hook86% 到         ["%_osppPath86%\OSPPC.DLL"])

echo 符号链接重命名后的 sppcs.dll            ["%_hookPath%\sppcs.dll"]
...
if defined ierror (
set error=1
call :dk_color %Red% "安装 Ohook                        [%ierror% 失败]"
echo:
call :oh_checkapps
if defined checknames (
call :dk_color %Blue% "关闭 [!checknames!] 并重试。"
...
if /i not "%ierror%"=="复制" call :dk_color %Blue% "使用重启选项重新启动您的机器并重试。"
if /i "%ierror%"=="复制" call :dk_color %Blue% "如果您使用的是第三方防病毒软件，请检查它是否阻止了脚本。"
)
echo:
)
...
set error=1
set ierror=1
call :dk_color %Red% "修改自定义 sppcs.dll 的哈希      [失败]"
) else (
echo 修改自定义 sppcs.dll 的哈希      [成功]
)
)
...
set error=1
call :dk_color %Red% "在脚本中检查产品              [Office %oVer%.0 !_prod! 密钥未在脚本中找到]"
...
::  如果在 Windows Server 上安装了零售版 Office C2R，则添加 SharedComputerLicensing 注册表项
::  https://learn.microsoft.com/zh-cn/office/troubleshoot/office-suite-issues/click-to-run-office-on-terminal-server
...
echo 添加 SharedComputerLicensing 注册表      [成功] [在安装了零售版 Office 的服务器上需要]"
)
)
...
echo 检查已安装的产品             [找到 SingleImage 产品，将使用 Professional Retail 密钥进行激活]
)
...
::  查找 Office vNext/shared/device 许可证块的残留并将其删除，因为它会阻止其他许可证出现
::  https://learn.microsoft.com/zh-cn/office/troubleshoot/activation/reset-office-365-proplus-activation-state
...
::  清除可能阻止 ohook 激活的 vNext/shared/device 许可证块
...
echo:
call :dk_color %Gray% "登录的 Office 帐户具有订阅许可证。"
call :dk_color %Blue% "如果订阅有效，它将覆盖其他激活方法。"
call :dk_color %Blue% "如果它即将过期，请在其过期后重新运行激活脚本。"
call :dk_color2 %Blue% "如果它已过期且激活失败，请在此处获取帮助 - " %_Yellow% " %mas%troubleshoot"
echo:
)

::  清除 Office 的 SharedComputerLicensing
::  https://learn.microsoft.com/zh-cn/deployoffice/overview-shared-computer-activation
...
::  清除基于设备的许可
::  https://learn.microsoft.com/zh-cn/deployoffice/device-based-licensing
...
::  删除 OEM 注册表项
::  https://support.microsoft.com/zh-cn/office/office-repeatedly-prompts-you-to-activate-on-a-new-pc-a9a6b05f-f6ce-4d1f-8d49-eb5007b64ba1
...
echo 清除 Office 许可证块          [已成功从所有 %counter% 个用户帐户中清除]

::==========================

::  某些零售产品尝试验证许可证，并可能显示横幅“检查此设备的许可证状态时出现问题”。
::  Resiliency 注册表项可以跳过此检查
...
echo 添加注册表以跳过许可证检查   [已成功添加到所有 %counter% 个及未来的新用户帐户]
)
...
if %upk_result%==2 (
call :dk_color %Red% "卸载其他/宽限期密钥           [失败]"
if not defined showfix (
call :dk_color %Blue% "%_fixmsg%"
echo:
set showfix=1
...
::  在 Insider 版本中需要此操作，否则 Office 可能无法激活
...
::  第 1 列 = Office 版本号
::  第 2 列 = 激活 ID
::  第 3 列 = 对于 Office 2013 及更高版本，列出了生成的密钥。对于 Office 2010，列出了从 Internet 获取的被阻止密钥。
::               对于 Office 2013 及更高版本，密钥优先级如下：零售:TB:订阅 > 零售 > OEM:NONSLP > 批量:MAK > 批量:GVLK
::               对于 Office 2010，密钥优先级如下：零售 > 批量:MAK
::  第 4 列 = 许可证描述的最后一部分
::  第 5 列 = 版本
::  第 6 列 = 如果它们是同一主要产品的一部分，则为其他版本 ID（仅供参考）
::  分隔符  = "_"

::===============

:: 我们找不到这些 Office 2010 产品的任何密钥（被阻止的或通用的都没关系）。如果您有，请与我们分享。
...
::  这样做是为了降低防病毒软件误报的可能性。每次安装时，它将安装一个唯一的 sppc dll 文件
...
# 如果校验和不匹配，则更新 MemoryStream 中的校验和
...
::
::  下面的文本块是 base64 编码格式
::  标签 "sppc32.dll" 和 "sppc64.dll" 中的块包含以下文件
::
...
::
::  文件以 base64 编码，使其成为 AIO 版本。
::
::  mass{}grave{dot}dev/ohook
::  在这里您可以找到文件源代码以及如何重建相同的 sppc.dll 文件的信息
::
::  stackoverflow.com/a/35335273
::  在这里您可以查看如何从 base64 提取 sppc.dll 文件
::
::  如有任何进一步问题，请随时通过 mass{}grave{dot}dev/contactus 与我们联系
::
::========================================================================================================================================
::
::  在进行 base64 转换之前，将 "-" 替换为 "A"，将 "_" 替换为 "a"
::  这样做是为了防止防病毒软件检测和标记 base64 编码
...
--CdV---41Q--C1V--BhVQ--n1U--NNV---NVg--O1Y--HFW--CvVg--z1Y--PtW--CIE---UVc--G9X--CfVw--01c--BFY--BNW---b1g--KVY--DNW---BVk--EFZ--BtWQ--p1k--LtZ--D7WQ--OVo--E9_--B1Wg--nWo--NN_---HWw--PVs--Glb--ClWw--
...
::  要激活 Windows，请使用 "/Z-Windows" 参数运行脚本或将下面行中的 0 改为 1
set _actwin=0

::  要激活 Windows ESU，请使用 "/Z-ESU" 参数运行脚本或将下面行中的 0 改为 1
set _actesu=0

::  要激活所有 Office 应用（包括 Project/Visio），请使用 "/Z-Office" 参数运行脚本或将下面行中的 0 改为 1
set _actoff=0

::  要仅激活 Project/Visio，请使用 "/Z-ProjectVisio" 参数运行脚本或将下面行中的 0 改为 1
set _actprojvis=0

::  要激活所有 Windows/ESU/Office，请使用 "/Z-WindowsESUOffice" 参数运行脚本或将下面行中的 0 改为 1
set _actwinesuoff=0

::  高级选项：
::  要激活 Windows K-M-S 主机（csvlk），请使用 "/Z-WinHost" 参数运行脚本或将下面行中的 0 改为 1
set _actwinhost=0

::  要激活 Office K-M-S 主机（csvlk），请使用 "/Z-OffHost" 参数运行脚本或将下面行中的 0 改为 1
set _actoffhost=0

::  要激活 Windows 8/8.1 APPX 旁加载，请使用 "/Z-APPX" 参数运行脚本或将下面行中的 0 改为 1
set _actappx=0

::  要激活特定的激活 ID，请将下面行中的 0 改为 1，并在 "tsids" 变量中设置激活 ID，您可以添加多个，用空格分隔
::  或使用 "/Z-ID-ActivationIdGoesHere" 参数运行脚本。如果要通过参数添加多个，请分别在单独的参数中传递
set _actman=
set tsids=

::  要重置重设计数器、评估期并清除篡改状态、密钥锁定，请使用 "/Z-Reset" 参数运行脚本或将下面行中的 0 改为 1
set _resall=0

::  选择激活方法：
::  在内部版本 26100 及更高版本中，脚本将自动选择 StaticCID（需要互联网）。如果未检测到互联网，它将自动选择 KMS4k 方法。对于低于 26100 的内部版本，脚本将自动选择 ZeroCID。
::  要更改激活方法，请使用参数 "/Z-SCID"、"/Z-ZCID" 或 "/Z-KMS4k" 运行脚本，或将下面行中的选项从 Auto 更改为 SCID、ZCID 或 KMS4k。
set _actmethod=Auto

::  调试模式：
::  要以调试模式运行脚本，请将下面行中的 0 更改为您要运行的上述任何参数
set "_debug=0"

::  如果使用了参数或更改了上述行中的值，脚本将以无人值守模式运行。
::  如果选择了多个选项，脚本将仅从高级选项中选取一个。
...
echo                  内部版本 ^>= 26100 - 仅 Windows  - KMS4k
echo                                    其他选项 - StaticCID
echo:
...
echo:
echo              [3] ZeroCID
echo                  在低于 26100 的版本上可靠工作
echo                  不适用于高于 26100.4188 的版本
echo              __________________________________________________
echo:
echo              [4] KMS4k
echo                  仅限批量许可证
...
reg query "HKLM\SOFTWARE\Microsoft\NET Framework Setup\NDP\v3.5" /v Install %nul2% | find /i "0x1" %nul1% || (
%eline%
echo .NET 3.5 Framework 已损坏或缺失。正在中止...
...
%eline%
echo 评估版 WLMS 服务正在运行，%_slser% 服务无法停止。正在中止...
echo 为 Windows 内部版本 %winbuild% 安装非评估版。
set fixes=%fixes% %mas%troubleshoot
call :dk_color2 %Blue% "请查看此网页获取帮助 - " %_Yellow% " %mas%troubleshoot"
goto dk_done
)
...
if /i !tsmethod!==KMS4k (
call :dk_color %Red% "检查互联网连接            [未连接]"
call :dk_color %Blue% "切换到 KMS4k 激活，因为 StaticCID 方法需要互联网。"
) else (
set error=1
call :dk_color %Red% "检查互联网连接            [未连接]"
...
if defined _perm (
call :dk_color %Gray% "检查 OS 激活                  [Windows 已永久激活]"
goto :ts_esu
)
...
if /i %tsmethod%==KMS4k (
if /i %_actmethod%==Auto (
call :dk_color %Blue% "返回上一菜单并选择 StaticCID 激活方法。需要互联网连接才能激活。"
) else (
call :dk_color %Blue% "从上一菜单使用非 KMS4K 激活选项。"
)
)
goto :ts_esu
)
...
if defined winsub (
call :dk_color %Blue% "Windows 订阅 [SKU ID-%slcSKU%] 已找到。脚本将激活基础版本 [SKU ID-%regSKU%]。"
echo:
)
...
::  用于 KMS4k 方法的 KMS 密钥，因为 TSforge 无法在 Windows Vista 和 7 上安装 KMS 密钥
...
call :dk_color %Blue% "从上一菜单使用 ZeroCID 激活方法。"
goto :ts_esu
)
...
if not defined key (
set error=1
call :dk_color %Red% "检查激活 ID                  [在系统中未找到 %tsedition% SKU-%osSKU%]"
call :dk_color %Blue% "%_fixmsg%"
goto :ts_esu
)
...
call :dk_color %Gray% "检查 OS 版本                     [%tsedition%] [发现评估版本]"
call :dk_color %Blue% "评估版本无法在其评估期之外激活。"

if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-Server*Edition~*.mum" (
call :dk_color %Blue% "脚本将重置评估期，但要永久激活 Windows，"
call :dk_color %Blue% "请返回主菜单并使用 [更改版本] 选项，更改为非评估版本。"
) else (
...
if not defined key (
set error=1
call :dk_color %Red% "检查激活 ID                  [在脚本中未找到 %tsedition%]"
...
REM 这是 Server-ESU-PA 的非通用被阻止 MAK 密钥
call :dk_inskey "[!key!]"
goto :ts_off
)
...
if defined isltsc (
call :dk_color %Gray% "检查激活 ID                  [%tsedition% LTSC 已有更长的支持，ESU 不适用]"
goto :ts_off
)
...
call :dk_color %Blue% "返回主菜单，选择更改 Windows 版本选项，然后更改为以下所列的任何版本。"
echo [%esueditionlist%]
goto :ts_off
)
...
call :dk_color2 %Blue% "请查看此网页获取帮助 - " %_Yellow% " %mas%tsforge#windows-esu"
) else (
...
if defined ohook (
echo:
call :dk_color %Gray% "检查 Ohook                          [Ohook 激活已为 Office 安装]"
)
...
if defined o14msi call :dk_color %Blue% "请改用 Ohook 激活选项来激活 Office 2010。"
)
...
if /i %tsmethod%==KMS4k (
echo:
call :dk_color %Red% "跳过 Windows %KS% 主机               [KMS4k 方法不支持 Windows %KS% 主机]"
if /i %_actmethod%==Auto (
call :dk_color %Blue% "连接到互联网并重试。脚本将使用 StaticCID 激活方法。"
) else (
call :dk_color %Blue% "从上一菜单使用非 KMS4K 激活选项。"
...
call :dk_color %Gray% "使用 %KS% 主机许可证，系统可能会在稍后随机更改 Windows 版本。这是一个 Windows 问题，可以安全地忽略。"
)
call :dk_color %Gray% "%KS% 主机 [不要与 %KS% 客户端混淆] 许可证会导致 %_slser% 服务持续运行。"
call :dk_color %Blue% "仅在必要时使用此激活，您可以从上一菜单恢复到正常激活。"
...
::  安装当前版本的 csvlk 许可证，以便正确反映 csvlk 的版本
...
call :dk_color %Red% "检查激活 ID                  [未找到] [%tsedition%] [%osSKU%]"
call :dk_color %Blue% "您的系统上未找到 %KS% 主机许可证。它适用于以下版本。"
...
if /i %tsmethod%==KMS4k (
echo:
call :dk_color %Red% "跳过 Office %KS% 主机                [KMS4k 方法不支持 Office %KS% 主机]"
if /i %_actmethod%==Auto (
call :dk_color %Blue% "连接到互联网并重试。脚本将使用 StaticCID 激活方法。"
) else (
call :dk_color %Blue% "从上一菜单使用非 KMS4K 激活选项。"
...
call :dk_color %Gray% "检查激活 ID                  [未找到 Office %KS% 主机]"
call :dk_color2 %Blue% "请查看此网页获取帮助 - " %_Yellow% " %mas%tsforge#office-kms-host"
)
...
::  处理 Windows 8/8.1 APPX 旁加载

echo:
echo 正在处理 Windows 8/8.1 APPX 旁加载...
...
call :dk_color %Red% "检查激活 ID                  [未找到]"
call :dk_color %Blue% "APPX 旁加载功能仅在专业版及更高版本上可用。"
)
...
echo:
if defined _vis (
echo 正在处理 Rearm / 计时器的重置...
) else (
echo 正在处理 Rearm / 计时器 / 篡改 / 锁定的重置...
)
echo:
...
if /i %tsmethod%==KMS4k (
echo:
call :dk_color %Red% "跳动手动激活              [KMS4k 方法不支持]"
if /i %_actmethod%==Auto (
call :dk_color %Blue% "连接到互联网并重试。脚本将使用 StaticCID 激活方法。"
) else (
call :dk_color %Blue% "从上一菜单使用非 KMS4K 激活选项。"
...
call :dk_color %Blue% "某些激活 ID 可能导致系统崩溃 [MUI 不匹配]，或不可逆的更改 [CloudEdition 等]。"
...
call :dk_color %Blue% "尽管脚本会尝试从列表中删除这些 ID，但不能完全保证。"
echo:
choice /C:0F /N /M "> [0] 返回  [F] 继续 : "
if %errorlevel%==1 exit /b
...
if defined _vis (
echo:
call :dk_color %Blue% "在 Windows Vista 和 Server 2008 上，您必须在激活前手动安装密钥。"
)
echo:
call :dk_color %Gray% "在打开的文本文件中输入/粘贴第一列所示的激活 ID，或直接按 Enter 返回："
echo 如果要添加多个，请在每个激活 ID 后添加空格：
echo:
set /p tsids=
...
if defined actdone (
call :dk_color %Green% "[%winos%] 已重置并成功激活，剩余 !gprdays! 天。"
) else (
set error=1
set showfix=1
...
call :dk_color %Blue% "已知此 Windows 版本因 MS Windows/Server 问题而无法激活。"
)
...
echo:
call :dk_color %Gray% "在 Windows 设置中，您可能会看到激活续订通知，可以忽略它。"
if /i %_actmethod%==Auto call :dk_color %Gray% "要避免此通知，请在有互联网连接的情况下运行脚本以使用 StaticCID 方法。"
)
echo "%tsids%" | find /i "7e94be23-b161-4956-a682-146ab291774c" %nul1% && (
call :dk_color %Gray% "Windows 更新可获得 1-3 年的 ESU；4-6 年是非官方的，但可让您手动安装 Server 2008 R2 更新。"
)
echo "%tsids%" | findstr /i "4afc620f-12a4-48ad-8015-2aebfbd6e47c 11be7019-a309-4763-9a09-091d1722ffe3" %nul1% && (
call :dk_color %Gray% "ESU 在 Windows 8.1 上不受官方支持，但可以手动安装更新，直到 2024 年 1 月。"
)
echo "%tsids%" | findstr /i "83d49986-add3-41d7-ba33-87c7bfb5c0fb 0b533b5e-08b6-44f9-b885-c2de291ba456" %nul1% && (
call :dk_color %Gray% "Windows 更新可获得 1-3 年的 ESU；4-6 年是非官方的，但可让您手动安装 LTSC 更新。"
...
echo:
call :dk_color %Gray% "Word、Excel 等 Office 应用已激活，请直接使用它们。忽略 Office 仪表板应用中的“购买”按钮。"
)

REM 触发 SPP 计划任务的重新评估
call :dk_reeval %nul%
)
...
echo:
echo 相反，它会将数据附加到软件保护平台使用的数据文件之一。
echo:
...
set skipprocess=1
call :dk_color %Gray% "跳过，因为处于 Project/Visio 模式     [%%#]"
)
)
...
call :ks_osppready
echo 转换不支持的 O365 Office      [%%# 为 MondoRetail]
...
set _altoffid=MondoRetail
call :ks_osppready
echo 转换不支持的 OEM-ARM Office   [%%# 为 MondoRetail]
)
)
)
...
echo 转换零售版到批量版             [%%# 为 !_altoffid!]
) else (
set _License=MondoVolume
set _altoffid=MondoVolume
echo 转换零售版到批量版             [%%# 为 !_altoffid!] [使用 Mondo，因为在脚本中未找到 %%#]
)
...
此代码改编自 ActivationWs。
原始仓库：https://github.com/dadorner-msft/activationws

MIT 许可证

版权所有 (c) Daniel Dorner

特此授予任何获得本软件及相关文档文件（“软件”）副本的人免费许可，
以不受限制地处理本软件，包括但不限于使用、复制、修改、合并、发布、
分发、再许可和/或销售本软件的副本，并允许本软件的接收者这样做，
但须符合以下条件：
...
持有人均不对因使用本软件或与本软件有关的任何索赔、损害或其他责任
负责，无论是合同诉讼、侵权行为还是其他行为。
...
using System.Xml.Linq;
    ...
    public static class ActivationHelper {
        ...
        private static XDocument CreateSoapRequest(int requestType, string installationId, string extendedProductId) {
            ...
            // 获取 activationRequest 的 Unicode 字节数组并将其转换为 Base64。
        ...
        private static string ParseSoapResponse(XDocument soapResponse) {
            ...
            try {
                ...
                if (responseXml.Descendants(BatchActivationResponseNs + "ErrorCode").Any()) {
                    ...
                    switch (errorCode) {
                        case "0x7F":
                            throw new Exception("多次激活密钥已达到其限制");

                        case "0x67":
                            throw new Exception("产品密钥已被阻止");

                        case "0x68":
                            throw new Exception("无效的产品密钥");

                        case "0x86":
                            throw new Exception("无效的密钥类型");
...
namespace LibTSforge.SPP
{
    ...
    public class ProductConfig
    {
        ...
        private List<KeyRange> GetPkeyRanges()
        {
            if (Ranges.Count == 0)
            {
                throw new ArgumentException("没有密钥范围。");
            }
            ...
            {
                throw new NotSupportedException("指定的激活 ID 仅适用于 Windows Anytime Upgrade。请使用非 WAU 激活 ID。");
            }
...
namespace LibTSforge.SPP
{
    ...
    public static class SPPUtils
    {
        public static void KillSPP(PSVersion version)
        {
            ...
            Logger.WriteLine(string.Format("正在停止 {0}...", svcName));
            ...
            for (int i = 0; stopped == false && i < 1080; i++)
            {
                ...
                {
                    Logger.WriteLine("警告：停止 sppsvc 失败，重试。详细信息：" + ex.Message);
            ...
            if (!stopped)
                throw new System.TimeoutException(string.Format("无法停止 {0}", svcName));

            Logger.WriteLine(string.Format("{0} 已成功停止。", svcName));
            ...
            if (version == PSVersion.Vista && SPSys.IsSpSysRunning())
            {
                ...
                if (status < 0)
                {
                    throw new IOException("无法卸载 spsys");
                }
        ...
        public static void RestartSPP(PSVersion version)
        {
            if (version == PSVersion.Vista)
            {
                ...
                for (int i = 0; started == false && i < 360; i++)
                {
                    ...
                    {
                        Logger.WriteLine("警告：启动 slsvc 失败，重试。详细信息：" + ex.Message);
                        System.Threading.Thread.Sleep(500);
                        continue;
                    }
                ...
                if (!started)
                    throw new System.TimeoutException("无法启动 slsvc");
        ...
        public static string GetPSPath(PSVersion version)
        {
            switch (version)
            {
                ...
                default:
                    ...
                    throw new FileNotFoundException("无法找到物理存储。");
            }
        }
        ...
        public static string GetTokensPath(PSVersion version)
        {
            switch (version)
            {
                ...
                default:
                    ...
                    throw new FileNotFoundException("无法找到令牌存储。");
            }
        }
        ...
        public static void DumpStore(PSVersion version, bool production, string filePath, string encrFilePath)
        {
            ...
            try
            {
                ...
                Logger.WriteLine("存储已成功转储到 '" + filePath + "'。");
            }
            finally
            {
                if (manageSpp)
...
namespace LibTSforge.Crypto
{
    ...
    public static class PhysStoreCrypto
    {
        public static byte[] DecryptPhysicalStore(byte[] data, bool production, PSVersion version)
        {
            ...
            byte[] hmacKey = decData.Take(0x10).ToArray(); // 在 Vista 上是 SHA-1 salt
            byte[] hmacSig = decData.Skip(0x10).Take(0x14).ToArray(); // 在 Vista 上是 SHA-1 哈希
            ...
            if (version != PSVersion.Vista)
            {
                ...
                {
                    throw new InvalidDataException("HMAC 验证失败。物理存储已损坏。");
                }
            }
            else
            {
                ...
                {
                    throw new InvalidDataException("校验和验证失败。物理存储已损坏。");
                }
            }
...
namespace LibTSforge.Modifiers
{
    ...
    public static class GenPKeyInstall
    {
        ...
        {
            if (version == PSVersion.Vista) throw new NotSupportedException("此功能不支持 Windows Vista/Server 2008。");
            ...
            if (config == null) throw new ArgumentException("在 PKeyConfig 中未找到激活 ID " + actId + "。");
            ...
            if (pkey.Algorithm == PKeyAlgorithm.PKEY2009)
            {
                ...
                Logger.WriteLine(string.Format("安装生成的产品密钥 {0} 状态 {1:X}", pkey, status));

                if ((int)status < 0)
                {
                    throw new ApplicationException("安装生成的产品密钥失败。");
                }
            ...
            Logger.WriteLine("密钥范围为 PKEY2005，正在创建虚假密钥数据...");
...
namespace LibTSforge.Modifiers
{
    ...
    public static class KMSHostCharge
    {
        public static void Charge(PSVersion version, bool production, Guid actId)
        {
            ...
            {
                throw new NotSupportedException("未安装 Volume:CSVLK 产品密钥。");
            }
            ...
            using (IPhysicalStore store = SPPUtils.GetStore(version, production))
            {
                ...
                Logger.WriteLine(string.Format("已成功将计数设置为 {0}。", currClients));
            }
...
namespace LibTSforge.Activators
{
    ...
    public class KMS4k
    {
        public static void Activate(PSVersion version, bool production, Guid actId)
        {
            ...
            {
                throw new NotSupportedException("未安装 Volume:GVLK 产品密钥。");
            }
                ...
                else
                {
                    ...
                    switch (version)
                    {
                        ...
                        case PSVersion.WinModern:
                            ...
                            break;
                        default:
                            throw new NotSupportedException("不支持的 PSVersion。");
                    }
            ...
            Logger.WriteLine("已使用 KMS4k 成功激活。");
        }
    }
}
...
namespace LibTSforge.Activators
{
    ...
    public static class ZeroCID
    {
        private static void Deposit(Guid actId, string instId)
        {
            ...
            Logger.WriteLine(string.Format("存入虚假 CID 状态 {0:X}", status));
        ...
        public static void Activate(PSVersion version, bool production, Guid actId)
        {
            ...
            {
                throw new NotSupportedException("此产品没有可用的电话许可证。");
            }
            ...
            SPPUtils.RestartSPP(version);
            SLApi.FireStateChangedEvent(appId);
            Logger.WriteLine("已使用 ZeroCID 成功激活。");
        }
    }
}
            ...
            do
            {
                ...
                for (int i = 0; i < ENTRIES_PER_BLOCK; i++)
                {
                    ...
                    if (populated)
                    {
                        ...
                        {
                            throw new FormatException("令牌内容中的数据长度与条目不一致。");
                        }
...
# 较旧的 .NET Framework 版本没有该方法，但它们也不会创建该文件夹。
}
        ...
        try {
            ...
            if ($activated) {
                if ($prodDes -match 'KMS' -and $prodDes -notmatch 'CLIENT') {
                    ...
                    Write-Host "[$prodName] CSVLK 已为 25 个客户端充值，有效期 30 天。" -ForegroundColor White -BackgroundColor DarkGreen
                }
                else {
                    if ($env:tsmethod -eq "KMS4k") {
                        if ($build -ge 26100) {
                            Write-Host "[$prodName] 已使用 KMS4k 激活，有效期超过 4000 年。" -ForegroundColor White -BackgroundColor DarkGreen
                            Write-Host "从内部版本 26100.7019 开始，Windows 在“设置”中将始终显示剩余激活期为 180 天。" -ForegroundColor White -BackgroundColor Darkgray
                        }
                        else {
                            Write-Host "[$prodName] 已使用 $env:tsmethod 激活，有效期至 $([DateTime]::Now.AddMinutes($GracePeriodStatus).ToString('yyyy-MM-dd HH:mm:ss'))。" -ForegroundColor White -BackgroundColor DarkGreen
                        }
                    }
                    else {
...
::  第 1 列 = Office 版本
::  第 2 列 = 批量版或免费零售产品
::  第 3 列 = 需要转换为第 2 列中提到的批量版产品的零售产品名称
::  分隔符  = "_"
...
::  要使用 K-M-S 激活 Windows，请使用 "/K-Windows" 参数运行脚本或将下面行中的 0 改为 1
set _actwin=0

::  要使用 K-M-S 激活所有 Office 应用（包括 Project/Visio），请使用 "/K-Office" 参数运行脚本或将下面行中的 0 改为 1
set _actoff=0

::  要仅使用 K-M-S 激活 Project/Visio，请使用 "/K-ProjectVisio" 参数运行脚本或将下面行中的 0 改为 1
set _actprojvis=0

::  要使用 K-M-S 激活所有 Windows/Office，请使用 "/K-WindowsOffice" 参数运行脚本或将下面行中的 0 改为 1
set _actwinoff=0

::  如果当前版本不支持 K-M-S 激活，要禁用更改 Windows/Office 版本，请使用 "/K-NoEditionChange" 参数运行脚本或将下面行中的 0 改为 1
set _NoEditionChange=0

::  要禁止随激活自动安装续订任务，请使用 "/K-NoRenewalTask" 参数运行脚本或将下面行中的 0 改为 1
set _norentsk=0

::  要卸载 K-M-S，请使用 "/K-Uninstall" 参数运行脚本或将下面行中的 0 改为 1。它将优先于任何其他参数。
set _uni=0

::  高级选项：
::  如果您要使用特定服务器名称而不是脚本中使用的公共服务器，请不要使用续订任务选项。

::  要指定用于激活的服务器地址，请使用 "/K-Server-YOURKMSSERVERNAME" 参数运行脚本或将其添加到下面行中的 = 号之后
set _server=

::  要指定用于激活的端口，请使用 "/K-Port-YOURPORTNAME" 参数运行脚本或将其添加到下面行中的 = 号之后
set _port=

::  如果使用了参数或更改了上述行中的值（用于激活或卸载），脚本将以无人值守模式运行。
...
call :dk_color2 %_White% "              [5] 随激活创建续订任务        " %_Yellow% "[否]"
)
...
echo        ______________________________________________________________
echo:
call :dk_color2 %_White% "       " %_Green% "使用键盘选择菜单选项 [1,2,3,4,5,6,7,8,9,0]"
...
set error=1
call :dk_color %Red% "检查互联网连接            [未连接]"
call :dk_color %Blue% "在线 %KS% 激活需要互联网。"
)
...
if defined _eval (
reg query "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion" /v EditionID %nul2% | find /i "Eval" %nul1% && (
call :dk_color %Red% "检查评估版本             [评估版本无法在其评估期之外激活。]"
...
call :dk_color %Blue% "Windows 必须更新到内部版本 19044.2788 或更高版本才能进行 IotEnterpriseS %KS% 激活。"
)
)

if not defined key if defined notfoundaltactID (
call :dk_color %Red% "检查备用版本的 %KS%      [%altedition% 激活 ID 未找到]"
)
...
if defined changekey (
call :dk_color %Blue% "将使用 [%altedition%] 版本产品密钥来启用 %KS% 激活。"
echo:
)
...
::  选择退出向 Microsoft 发送 KMSclient 激活数据
::  https://learn.microsoft.com/zh-cn/windows/privacy/manage-connections-from-windows-operating-system-components-to-microsoft-services#19-software-protection-platform
...
call :dk_color %Red% "无法获取已安装的批量 Windows / Office 产品。"
)
call :_taskgetserv
call :_taskregserv
)
...
::  如果未找到 Windows/Office 批量 ID，即使脚本默认设置为创建续订任务，也不要创建它
::  如果仅找到 Windows 批量 ID 并且在 Windows 7 上存在 OEM BIOS 错误，即使脚本默认设置为创建续订任务，也不要创建它
...
if exist "%ProgramFiles%\Activation-Renewal\Activation_task.cmd" call :dk_color %Gray% "正在删除激活续订任务..."
call :dk_color %Gray% "跳过创建激活续订任务..."
...
::  https://learn.microsoft.com/zh-cn/azure/virtual-desktop/windows-10-multisession-faq
...
set _server=
echo:
echo 输入/粘贴 %KS% 服务器地址，或直接按 Enter 返回：
echo:
set /p _server=
...
echo:
echo 输入/粘贴 %KS% 端口地址，或直接按 Enter 使用默认值：
echo:
set /p _port=
...
::  零售版转换为批量版后，新产品 ID 需要在注册表中添加 .OSPPReady 键，否则产品信息可能无法完全反映
...
::  Office 内部版本 16.0.10730.20102 之前的版本需要在 ProductReleaseIds 中包含已安装的许可证产品 ID，否则产品信息可能无法完全反映
...
set skipprocess=1
call :dk_color %Gray% "跳过，因为处于 NoEditionChange 模式     [%%#]"
)
)
...
echo 转换零售版到批量版             [!_prod! 为 !_altoffid!]
echo %%# | find /i "O365" %nul% && (
if "%oVer%"=="15" (call :dk_color %Gray% "Mondo 2013 在最新功能方面等同于 O365 [15.0 版本]。")
if "%oVer%"=="16" (call :dk_color %Gray% "Mondo 2016 在最新功能方面等同于 O365。")
)
...
if not defined _oMSI (
set error=1
call :dk_color %Red% "在脚本中检查产品              [Office %oVer%.0 !_prod! 未在脚本中找到]"
call :dk_color %Blue% "请确保您使用的是最新的 MAS 脚本。"
) else (
call :dk_color %Red% "正在检查脚本中的产品              [!_prod! MSI 零售版不受支持]"
call :dk_color %Blue% "请使用 Ohook 选项来激活它。要使用 %KS% 激活，您需要安装 Office 的批量许可版本。"
)
set fixes=%fixes% %mas%genuine-installation-media
call :dk_color %_Yellow% "%mas%genuine-installation-media"
)
...
::   通过计划任务使用在线服务器续订 K-M-S 激活

::============================================================================
::
::   主页：m{}assgrave{dot}dev
::
::============================================================================
...
::  检查 Internet 连接。即使禁用了 ICMP 回显也能工作。
...
if defined t_name 正在激活：%prodname%
...
if "%1"=="act_win" if %_kms38% EQU 1 (
if defined t_name (
echo %prodname% 已通过 KMS38 激活。
) else (
call :dk_color %Green% "%prodname% 已通过 KMS38 激活。"
)
exit /b
)
...
if defined t_name (
echo %prodname% 激活失败，由于网络受限或无网络。
) else (
call :dk_color %Red% "%prodname% 激活失败，由于网络受限或无网络。"
)
set showfix=1
set _tserror=1
exit /b
...
if defined t_name (
echo %prodname% 无法在此计算机上通过 KMS 激活，因为 OEM BIOS 不合格 [0xC004F035]。
) else (
call :dk_color %Red% "%prodname% 无法在此计算机上通过 KMS 激活，因为 OEM BIOS 不合格 [0xC004F035]。"
call :dk_color %Blue% "请从主菜单使用 TSforge 激活选项。"
)
set oemerr=1
set showfix=1
exit /b
...
if defined t_name (
echo %prodname% 激活失败，由于网络问题 [0xC004F074]。
) else (
call :dk_color %Red% "%prodname% 激活失败，由于网络问题 [0xC004F074]。"
if not defined _tserror (
call :dk_color %Blue% "请确保系统文件未被防火墙阻止。"
call :dk_color %Blue% "如果问题仍然存在，请尝试其他网络连接或 VPN，例如 https://1.1.1.1"
)
)
set showfix=1
set _tserror=1
...
if defined t_name (
echo %prodname% 已成功激活，剩余 %gpr2% 天。
) else (
call :dk_color %Green% "%prodname% 已成功激活，剩余 %gpr2% 天。"
)
exit /b
)
...
if defined t_name (
echo %prodname% 激活失败 [0x!=ExitCode!]。剩余期限：%gpr2% 天 [%gpr% 分钟]。
) else (
...
:: 将特定的 K-M-S 主机设置为本地主机，这样全局 K-M-S IP 不能替换 KMS38 激活，但仍可用于 Office 和其他 Windows 版本。
...
if defined t_name echo 检查：未安装批量版本的 Windows
)
...
::  获取用于激活的 K-M-S 服务器的 IPv4 地址，即使禁用了 ICMP 回显也能工作。
::  如果直接使用公共 K-M-S 服务器主机名进行激活，Microsoft 和防病毒软件可能会标记问题。
...
if defined error_ (
set error=1
call :dk_color %Red% "无法删除以前的续订任务。请重启系统 / 重试。"
exit /b
)
...
rmdir /s /q "%_dest%\" %nul%
set error=1
call :dk_color %Red% "安装续订任务失败。请重启系统 / 重试。"
exit /b
)

if "%keyerror%"=="0" if not defined _tserror (
call :dk_color %Green% "终身激活的续订任务已成功安装在 %_dest%"
exit /b
)
echo 终身激活的续订任务已成功安装在 %_dest%
exit /b

::  从批处理脚本中提取文本，避免字符和文件编码问题
...
(
echo   此脚本的用途是使用在线 K-M-S 续订 Windows/Office 许可证。
echo:
echo   如果创建了续订/激活计划任务，则存在以下内容：
echo:
echo   - 计划任务
...
<Task version="1.3" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    ...
    <Version>1.0</Version>
    <Description>在线 K-M-S 激活续订 - 每周任务</Description>
...
<Task version="1.3" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    ...
    <Author>WindowsAddict</Author>
    <Version>1.0</Version>
    <Description>在线 K-M-S 激活一次性运行 - 首次网络连接时运行并自行删除</Description>
...
::  从 pkeyhelper.dll 获取产品密钥，用于未来的新版本
::  适用于 Windows 10 1803 (17134) 及更高版本。
...
::  第1列 = Office 版本号
::  第2列 = 激活 ID
::  第3列 = branding.xml 中的产品 ID
::  第4列 = 版本
::  第5列 = 如果同一主要产品包含其他版本 ID（仅供引用）
::  分隔符 = "_"
...
::  第1列 = 激活 ID
::  第2列 = GVLK / 免费 Office 产品密钥
::  第3列 = 对于 Windows，为 SKU ID；对于 Office，为 Office 版本
::  第4列 = 版本 ID
::  第5列 = 对于 Windows，为生成分支名称（如果同一版本 ID 在不同 OS 版本中使用不同密钥，仅供参考）
::              对于 Office，如果是免费 Office 产品则为密钥类型，或者是需要转换为第4列中版本 ID 的零售产品名称
::              在 Office 2010 中，从每个主要产品 ID 中选择一个最高级的 VL 版本，因此 Visio Prem 密钥被列出，但没有 Visio Pro、Std 的
::  分隔符 = "_"
...
::  如果当前版本不支持 K-M-S 激活，则获取备用版本名称和密钥
...
<#
;;; 来源：https://github.com/asdcorp/clic
;;; PowerShell 端口：abbodi1406

版权 2023 asdcorp

特此免费授予任何获得本软件及其相关文档文件（“软件”）副本的人，不受限制地处理本软件，包括但不限于使用、复制、修改、合并、发布、分发、再许可和/或销售本软件的副本，并允许被提供本软件的人员这样做，但须遵守以下条件：

上述版权声明和本许可声明应包含在本软件的所有副本或主要部分中。

本软件按“原样”提供，不提供任何明示或暗示的担保，包括但不限于适销性、特定用途适用性和非侵权性的担保。在任何情况下，作者或版权持有人均不对任何索赔、损害或其他责任负责，无论是在合同、侵权或其他行为中，由软件或软件的使用或其他交易引起、导致或与之相关。
#>
...
%eline%
echo 检测到不受支持的 OS 版本。
echo 此命令仅适用于 Windows 8/8.1/10/11 及其服务器等效版本。
goto :at_back
)
...
echo %line%
echo:
echo      DISM 使用 Windows 更新提供修复损坏所需的替换文件。
echo      这将花费 5-15 分钟或更长时间。
echo %line%
echo:
echo      注意：
echo:
call :dk_color2 %_White% "     - " %Gray% "请确保 Internet 已连接。"
...
echo:
call :dk_color %Gray% "CBS 和 DISM 日志已复制到桌面上的 AT_Logs 文件夹。"
goto :at_back
...
echo %line%
echo:    
echo      SFC 将修复缺失或损坏的系统文件。
echo      建议先运行 DISM 选项再运行此选项。
echo      这将花费 10-15 分钟或更长时间。
echo:
echo      如果 SFC 无法修复某些内容，请再次运行该命令，看看下次是否能修复。
echo      有时可能需要运行 sfc /scannow 命令 3 次，每次重启 PC，才能完全修复所有能修复的内容。
echo:   
echo %line%
echo:
choice /C:09 /N /M ">    [9] 继续 [0] 返回："
...
echo:
echo       - 此选项将：
echo            - 停用 Windows 和 Office，您可能需要重新激活。
echo              如果 Windows 使用主板 / OEM / 数字许可证激活，那么 Windows 将自动重新激活。
echo:
echo            - 清除 ClipSVC、SPP 和 OSPP 许可证。
echo            - 修复 SPP 令牌文件夹和注册表的权限。
echo            - 触发 Office 修复选项。
echo:
...
if %winbuild% LSS 10240 (
echo ClipSVC 许可证重建仅支持 Windows 10/11。
echo 正在跳过...
goto :rebuildspptok
)
...
::  下面的注册表键（易失且受保护）在 ClipSVC 许可证清理命令后创建，并在系统重启后自动删除。
::  需要删除它以在不重启的情况下激活系统。
...
echo:
echo 正在删除易失且受保护的注册表键...
echo [%RegKey%]
reg query "%RegKey%" %nul% && (
call :dk_color %Red% "[失败]"
echo 请使用重启选项重新启动计算机，这将自动删除此注册表键。
) || (
echo [成功]
)

::   清除 HWID 令牌相关注册表以修复激活中的任何损坏

echo:
echo 正在删除 IdentityCRL 注册表键...
echo [%_ident%]
reg delete "%_ident%" /f %nul%
reg query "%_ident%" %nul% && (
call :dk_color %Red% "[失败]"
...
echo:
echo 正在重启 wlidsvc 和 LicenseManager 服务...
...
call :dk_color %Red% "未找到 tokens.dat 文件。"
) else (
...
set badregistry=1
echo:
call :dk_color %Red% "TokenStore 注册表中未找到正确路径 [%tokenstore%]"
)
...
REM 修复 Windows 7 中 KB971033 更新引起的问题
...
echo:
if %counter% EQU 0 (
echo 未安装 Office ^(2010及更高版本^)。
goto :repairend
) else if not defined c2r16_68 if not defined c2r16_86 (
call :dk_color %_Yellow% "将出现一个新窗口，在该窗口中您需要选择 [快速修复] 选项。"
if defined terminal (
...
call :dk_color %Red% "[无响应]"
echo:
echo 请运行 [Dism 还原健康] 和 [SFC 扫描] 选项，并确保没有错误。
) else (
call :dk_color %Green% "[正常]"
)
...
::  此代码检查 SPP 是否具有对令牌文件夹和所需注册表键的访问权限。不正确的权限通常由游戏欺骗程序设置。

set permerror=
if not exist "%tokenstore%\" set "permerror=在令牌文件夹中发现错误"
...
if ($env:permerror -eq '在令牌文件夹中发现错误') {
...
if ($env:permerror -eq '在 SPP 注册表中发现错误') {
...
function Take-Permissions {
    ...
    ### 步骤 1 - 提升当前进程的权限
    # 在执行以下行之前获取 SeTakeOwnership、SeBackup 和 SeRestore 权限，脚本需要管理员权限
    $AssemblyBuilder = [AppDomain]::CurrentDomain.DefineDynamicAssembly(4, 1)
    $ModuleBuilder = $AssemblyBuilder.DefineDynamicModule(2, $False)
    $TypeBuilder = $ModuleBuilder.DefineType(0)
    ...
    function Take-KeyPermissions {
        ...
        ### 步骤 2 - 获取键的所有权 - 仅对当前键有效
        $regKey = [Microsoft.Win32.Registry]::$rootKey.OpenSubKey($key, 'ReadWriteSubTree', 'TakeOwnership')
        $acl = New-Object System.Security.AccessControl.RegistrySecurity
        $acl.SetOwner($sid)
        $regKey.SetAccessControl($acl)

        ### 步骤 3 - 启用当前键从其父级继承权限（不是所有权）
        $acl.SetAccessRuleProtection($false, $false)
        $regKey.SetAccessControl($acl)

        ### 步骤 4 - 仅对顶级键，更改当前键的权限并传播给子键
        # 要启用对子键的传播，需要对每个子键执行步骤 2-3（步骤 5）
...
::  要在使用 CBS 升级方法更改版本时暂存当前版本，请将下面行的 0 改为 1
set _stg=0
...
set "showeditionerror=call :dk_color %_Yellow% "[%osedition%] [注册表-%regedition%].""
)
...
echo:
echo 更改此版本可能不会删除 "%osedition%" 特定的功能。
echo:
...
%line%
echo:
if defined dismnotworking call :dk_color %Red% "DISM.exe 无法工作。"
...
call :dk_color %_Yellow% "注意 - DISM.exe 无法工作。"
if /i "%osedition:~0,4%"=="Core" call :dk_color %_Yellow% "     - 更改为 Pro 后，您将看到更多版本选项。"
)
%line%
echo:
...
echo [0]  %_exitmsg%
echo:
call :dk_color %_Green% "使用键盘输入选项编号并按 Enter 确认："
set /p inpt=
...
call :dk_color %Red% "==== 注意 ===="
echo:
echo 一旦版本更改为 "%targetedition%",
echo 系统可能无法在之后正确更改版本。
echo:
echo [1] 继续
echo [0] 返回
echo:
...
::  检查是否需要 DISM API 或 slmgr.vbs 进行版本升级
...
::  从 Core 更改为非 Core，以及在 Windows 版本低于 17134 时更改版本，需要使用 "changepk /productkey" 或 DISM API 方法并重启
::  在其他情况下，可以使用 "slmgr /ipk" 即时更改版本
...
echo:
%showeditionerror%
if defined dismnotworking call :dk_color %_Yellow% "DISM.exe 无法工作。"
...
if %_dismapi%==1 (
echo:
echo 正在使用 DISM API 方法应用 %_chan% 密钥 %key%。请稍候...
echo:
...
echo:
%showeditionerror%
if defined dismnotworking call :dk_color %_Yellow% "注意 - DISM.exe 无法工作。"
echo 正在将当前版本 [%osedition%] %fullbuild% 更改为 [%targetedition%]...
echo:
call :dk_color %Blue% "重要 - 继续之前请保存您的工作，系统将自动重启。"
echo:
...
echo:
call :dk_color %Blue% "如果出现错误，您应在再次尝试之前重启系统。"
echo:
...
echo 发现挂起的重启标志。
echo:
echo 请确保 Windows 已完全更新，重启系统并重试。
)
exit /b
...
if($null -eq $editionXml) {
    Write-Warning '无法找到版本特定的设置 XML。将在没有它的情况下继续...'
}
...
::  第1列 = 通用零售/OEM/MAK/GVLK 密钥
::  第2列 = 密钥类型
::  第3列 = WMI 版本 ID
::  第4列 = 版本名称（如果同一版本 ID 在不同 OS 版本中使用不同密钥）
::  分隔符 = _

::  对于 Windows 10/11 版本，尽可能列出 HWID 密钥；在服务器版本中，尽可能列出 KMS 密钥。
::  对于 Windows，通用密钥列出到 22000；对于 Server，通用密钥列出到 17763，后续版本从 pkeyhelper.dll 提取。
...
echo 正在中止...
call :dk_color %Blue% "请从以下 URL 下载并安装最新 Office，然后重试。"
set fixes=%fixes% %mas%genuine-installation-media
call :dk_color %_Yellow% "%mas%genuine-installation-media"
goto dk_done
)
...
%eline%
echo 安装的 Office 似乎来自批量许可通道 %ltsc19%%ltsc21%%ltsc24%，
echo 这在您的 Windows 版本 %winbuild% 上不受官方支持。
echo 正在中止...
set fixes=%fixes% %mas%troubleshoot
call :dk_color2 %Blue% "查看此网页获取帮助 - " %_Yellow% " %mas%troubleshoot"
goto dk_done
...
%eline%
echo 您的 Windows 版本 %winbuild% 上安装了不受支持的 Office %verchk%。
echo 正在中止...
set fixes=%fixes% %mas%troubleshoot
call :dk_color2 %Blue% "查看此网页获取帮助 - " %_Yellow% " %mas%troubleshoot"
goto dk_done
...
echo                 [1] Office 套件     - 零售
echo                 [2] Office 套件     - 批量
echo                 [3] Office 单个应用 - 零售
echo                 [4] Office 单个应用 - 批量
echo                 ____________________________________________
echo:
echo                 [0] 返回
echo         ____________________________________________________________
...
if %winbuild% LSS 10240 (
echo 2019/2021/2024 等不受支持的产品已从此列表中排除。
) else (
...
if defined _notfound call :dk_color %Gray% "Office !_notfound! 不在此列表中，因为安装的 Office 版本 [%_version%] 较旧。"
)
%line%
echo:
...
if defined suites (
find /i "%%#" "%SystemRoot%\Temp\%o_randguid%\getAppIds.txt" %nul1% && (set %%#_st=开启) || (set %%#_st=)
) else (
set %%#_st=
)
)

if defined Lync_st set Lync_st=关闭
set OneDrive_st=关闭
if defined suites (set Teams_st=关闭) else (set Teams_st=)

set OutlookForWindows_st=
if %winbuild% GEQ 19041 if defined Outlook_st echo %targetedition% | find /i "O365" %nul1% && (
set OutlookForWindows_st=关闭
)
...
call :dk_color %Gray% "要排除下面列出的应用，请将它们从“开启”切换为“关闭”。"
if defined editedition call :dk_color %Gray% "注意：下面的开启/关闭状态并不反映已安装应用的实际状态。"
%line%
...
if defined Outlook_st           echo [O] Outlook ^(经典版^)   : %Outlook_st%
...
if defined OutlookForWindows_st echo [K] Outlook ^(新版^)     : %OutlookForWindows_st%
...
if !_el!==13 if defined Teams_st             (if "%Teams_st%"=="关闭"             (set Teams_st=开启)             else (set Teams_st=关闭))
if !_el!==12 if defined OneDrive_st          (if "%OneDrive_st%"=="关闭"          (set OneDrive_st=开启)          else (set OneDrive_st=关闭))
if !_el!==11 if defined OutlookForWindows_st (if "%OutlookForWindows_st%"=="关闭" (set OutlookForWindows_st=开启) else (set OutlookForWindows_st=关闭))
if !_el!==10 if defined Lync_st              (if "%Lync_st%"=="关闭"              (set Lync_st=开启)              else (set Lync_st=关闭))
if !_el!==9  if defined Word_st              (if "%Word_st%"=="关闭"              (set Word_st=开启)              else (set Word_st=关闭))
if !_el!==8  if defined Visio_st             (if "%Visio_st%"=="关闭"             (set Visio_st=开启)             else (set Visio_st=关闭))
if !_el!==7  if defined Publisher_st         (if "%Publisher_st%"=="关闭"         (set Publisher_st=开启)         else (set Publisher_st=关闭))
if !_el!==6  if defined Project_st           (if "%Project_st%"=="关闭"           (set Project_st=开启)           else (set Project_st=关闭))
if !_el!==5  if defined PowerPoint_st        (if "%PowerPoint_st%"=="关闭"        (set PowerPoint_st=开启)        else (set PowerPoint_st=关闭))
if !_el!==4  if defined Outlook_st           (if "%Outlook_st%"=="关闭"           (set Outlook_st=开启)           else (set Outlook_st=关闭))
if !_el!==3  if defined OneNote_st           (if "%OneNote_st%"=="关闭"           (set OneNote_st=开启)           else (set OneNote_st=关闭))
if !_el!==2  if defined Excel_st             (if "%Excel_st%"=="关闭"             (set Excel_st=开启)             else (set Excel_st=关闭))
if !_el!==1  if defined Access_st            (if "%Access_st%"=="关闭"            (set Access_st=开启)            else (set Access_st=关闭))
goto :oe_excludeapps
...
if /i "!%%#_st!"=="关闭" if defined excludelist (set excludelist=!excludelist!,%%#) else (set excludelist=,%%#)
)
exit /b
...
::  检查 Project 和 Visio 是否使用不受支持的语言

set projvis=
set langmatched=
echo: %Project_st% %Visio_st% | find /i "开启" %nul% && set projvis=1
...
%eline%
echo %_lang% 语言不适用于 Project/Visio 应用。
call :dk_color %Blue% "请从以下 URL 安装支持 Project/Visio 的语言版本的 Office。"
set fixes=%fixes% %mas%genuine-installation-media
call :dk_color %_Yellow% "%mas%genuine-installation-media"
goto :oe_goback
)
...
::  感谢 @abbodi1406 首次发现 OfficeClickToRun.exe 的用法
::  感谢 @may 建议使用它通过 CDN 作为源更改版本
::  此处使用 OfficeClickToRun.exe 的 productstoadd 方法添加版本
::  它使用增量更新，意味着由于使用相同的已安装版本，将消耗很少的 Internet 流量
...
call :dk_color %Gray% "更新通道与已安装产品不匹配。"
call :dk_color %Blue% "建议从上一个菜单更改更新通道为 [!suggestchannel!]。"
)
echo:
)
...
%line%
echo:
call :dk_color %Gray% "您可以编辑 [添加/删除应用] 以下 Office 版本之一。"
%line%
echo:
...
echo 您的 Windows 版本 %winbuild% 上安装了不受支持的 Office 版本。
goto :oe_goback
)
if defined ltscfound (
%eline%
echo 已安装的 Office 更新通道：%ltsc19%%ltsc21%%ltsc24%
echo 您的 Windows 版本 %winbuild% 上安装了不受支持的 Office 更新通道。
goto :oe_goback
)
)
...
set /a counter+=1
if !counter! LSS 10 (
if defined bypass (echo [!counter!]  %%B  将使用非官方更改方法) else (echo [!counter!]  %%B)
) else (
if defined bypass (echo [!counter!] %%B  将使用非官方更改方法) else (echo [!counter!] %%B)
)
...
::  获取目标 FFN 的版本号，在 OfficeC2RClient.exe 命令中使用版本号触发更新可提供准确结果
...
if %winbuild% GEQ 9200 call :dk_color %Gray% "无法检测目标 FFN 的版本号。"
...
echo 非永久 Office 产品不支持永久 VL 更新通道。
goto :oe_goback
)
...
%eline%
echo [%%A] 语言在 Office 2019 Perpetual VL 更新通道上不受支持。正在中止...
goto :oe_goback
)
)
)
...
echo:
call :dk_color %Blue% "在操作完成之前请不要终止..."
echo:
...
if %clverchk% LSS %buildchk% (
echo:
call :dk_color %Red% "更新 Office C2R 客户端失败。正在中止..."
set fixes=%fixes% %mas%troubleshoot
call :dk_color2 %Blue% "查看此网页获取帮助 - " %_Yellow% " %mas%troubleshoot"
goto :oe_goback
)
...
::  清理 Office 更新相关注册表，感谢 @abbodi1406
...
echo:
echo 正在清理 Office 更新注册表键...
echo 正在将新更新通道添加到注册表键...
echo:
...
call :dk_color %Blue% "此操作需要 Internet。"
)
exit /b
...
::  从 c2rpridslicensefiles_auto.xml 获取可用版本列表
::  并使用 MasterDescriptor.x-none.xml 过滤列表
::  在 Windows 7/8/8.1 上排除不受支持的产品
...
::  使用 MasterDescriptor.x-none.xml 获取特定产品 ID 的应用列表
