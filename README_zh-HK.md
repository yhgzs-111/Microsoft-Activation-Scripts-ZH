...
<p align="center">一款開源的 Windows 與 Office 啟用工具，支援 HWID、Ohook、TSforge、KMS38 及網上 KMS 等多種啟用方式，並提供進階故障排除功能。</p>
...
Windows 和 Office 啟用工具 **MAS (Microsoft-Activation-Scripts)** 的漢化版，在不改變機器碼的情況下可永久啟用 (視啟用方式而定) 。

**最新版本 (Latest Version)：3.12**

### 漢化版注意事項

本漢化版本實現了MAS指令碼的完整漢化，註釋、互動提示等所有界面與程式碼細節均已中文化。既可用於日常啟用操作，也便於深入研習MAS指令碼的內在實現原理。

該漢化版本由[**Deepseek-R1**](https://deepseek.com/)進行初步漢化+[**本人**](https://github.com/yhgzs-111/)校對完成。

**請勿隨意修改指令碼編碼，否則指令碼有可能無法正常執行！！！**

由於不可抗力的編碼因素，極少數界面提示或註釋可能殘留輕微亂碼，但這完全不影響啟用功能及指令碼的正常執行。

下載連結均已進行**中國大陸加速**。

有關指令碼使用的更多資訊及常見問題，請參閱原指令碼官網[**massgrave.dev**](https://massgrave.dev/)。
...
- **Windows Defender** 有機會誤刪，使用前請關閉**即時保護**。
...
下載後按兩下開啟即可使用。如無法下載/開啟，請關閉 **Microsoft SmartScreen**。
...
🔍 [前往官方 Github 倉庫 (MAS Source Code)](https://github.com/massgravel/Microsoft-Activation-Scripts?tab=readme-ov-file#download--how-to-use-it)
...
感謝[**cmontage**](https://github.com/cmontage)的[**mas-cn**](https://github.com/cmontage/mas-cn)專案為本倉庫README提供範本，為本倉庫結構和.ps1指令碼提供借鑒。

**在此致謝！**

---

### 官方原版主頁 - [https://massgrave.dev/](https://massgrave.dev/)
